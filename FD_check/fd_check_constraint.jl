include("nonlcon.jl")
function fd_check_constraint(OPT::OPT_struct, FE::FE_struct)
    #
    ...
    theta_0, grad_theta_0, _, _ = nonlcon(OPT, FE, dv_0)
    # Finite differences
    println("Computing finite difference sensitivities of constraint...")
    # Do this for all design variables or only a few
    up_to_dv = n_dv

    for i in 1:up_to_dv
        dv_i[i] = dv_0[i] + fd_step
        theta_i, _, _, _ = nonlcon(OPT, FE, dv_i)
