...
function run_finite_difference_check(OPT::OPT_struct, FE::FE_struct, check_cost_sens, check_const_sens, path_output)
    ...
    if check_cost_sens == true && check_const_sens == true
        grad_cost_0, grad_cost_i = fd_check_cost(OPT::OPT_struct, FE::FE_struct)
        grad_const_0, grad_const_i = fd_check_constraint(OPT::OPT_struct, FE::FE_struct)
    else
        if check_cost_sens == true
            grad_cost_0, grad_cost_i = fd_check_cost(OPT::OPT_struct, FE::FE_struct)
        elseif check_const_sens == true
            grad_const_0, grad_const_i = fd_check_constraint(OPT::OPT_struct, FE::FE_struct)
        end
    end
    ...
    if check_cost_sens == true && check_const_sens == true
        ...
        # Saving plots
        Makie.save(path_output * OPT.examples *"_"*OPT.TPMS * "_FD_cost_" * OPT.functions.objective * ".png", fig_FD1)
        # ------
        fig_FD2 = Figure()
        ...
        # ------
        # Saving plots
        Makie.save(path_output * OPT.examples *"_"*OPT.TPMS * "_FD_const_" * OPT.functions.constraints * ".png", fig_FD2)
        # -----
