...
function runmma(OPT::OPT_struct, FE::FE_struct, x0, path_output)
    # Initialize history object
    history = History_Opt([], [], [], [])
    # -----------Adaptive constraint scaling
    ACS = ACS_Opt(true, 0.8, 1, [])

    if (OPT.functions.objective == "maximum_damage" || OPT.functions.objective == "maximum_damage_SGs"
        || OPT.functions.objective == "maximum_stress" || OPT.functions.objective == "maximum_stress_SGs"
        || OPT.functions.objective == "compliance" || OPT.functions.objective == "compliance"
    )
        ACS.use = false
    end
    ...
    iter = 0
    f0val, df0dx, fval, dfdx, OPT, FE = obj_nonlcon(OPT::OPT_struct, FE::FE_struct, x)
    ...
    while iter < OPT.options.max_iter
        ...
        f0val, df0dx, fval, dfdx, OPT, FE = obj_nonlcon(OPT::OPT_struct, FE::FE_struct, copy(xmma))
