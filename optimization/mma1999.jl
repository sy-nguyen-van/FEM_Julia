...
function mma1999(m, n, iter, xval, xmin, xmax, xold1, xold2, f0val, df0dx, df0dx2, fval, dfdx, dfdx2, low, upp, a0, a, c, d)
    #
    ...
    feps = 0.000001
    # asyinit = 0.5;
    # if strcmp(OPT.parameters.aggregation_type, 'mrf')
    #     asyinit = OPT.options.move_limit;
    # else
    asyinit = 0.5
    # end
    # Original values for increase and decrease factors: 1.2 and 0.7
    # Modified values for more conservative approximation (following Oest and
    # Lund (2007)) are 1.05 and 0.65.
    # asyincr=1.05;
    # asydecr = 0.65;
    asyincr = 1.2
    asydecr = 0.7
    ...
    if iter < 2.5
        ...
        factor = factor .* (asyincr * (zzz .>= 0) + asydecr * (zzz .< 0))
    ...
    p0 = max.(df0dx, 0) # Replace find function for efficienty
    ...
    Q = Q .+ delpos * spdiagm(n, n, 0 => diaq)
    b = P * uxinv1 .+ Q * xlinv1 .- fval

    ### Solving the subproblem by a primal-dual Newton method
    xmma, ymma, zmma, lam, xsi, eta, mu, zet, s = subsolv1999(m, n, epsimin, low, upp, alfa, beta, p0, q0, P, Q, a0, a, b, c, d)
