...
function setup_bcs_Vframe2d(OPT::OPT_struct, FE::FE_struct)
    ## Input file 
    #
    ...
    load_nodes = findall(Bool.(FE.coords[2, :] .> (H - tol)) .&& Bool.(abs.(FE.coords[1, :] .- W / 2) .< (lw + tol)))
    fixed_nodes = findall(Bool.(FE.coords[2, :] .< tol) .&& Bool.(FE.coords[1, :] .< (d + tol)))
    roller_nodes = findall(Bool.(FE.coords[2, :] .< tol) .&& Bool.(FE.coords[1, :] .> (W - d - tol)))

    FE = compute_predefined_node_sets(FE, ["BL_pt"]) #<<
    ...
    disp_dirs = vec([vec(disp_dirs_f2); vec(disp_dirs_r2); vec(disp_dirs_bl1)])
    ...
    length_disp_region = length(disp_region)
    for idisp in 1:length_disp_region
