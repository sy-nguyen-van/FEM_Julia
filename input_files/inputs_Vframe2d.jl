function inputs_Vframe2d(OPT::OPT_struct, FE::FE_struct)
  ## ============Input file================
  ...
  if OPT.fd_check == 1
    FE.mesh_input.box_dimensions = [60, 30] # 
    FE.mesh_input.cutout_dimensions = [55, 15]
    FE.mesh_input.elements_per_side = [15, 5] # 
  else
  ...
  OPT.aggregation_power = 12    # the power for the p-norm
   ...
   if OPT.fd_check == 1
    ...
    if OPT.run_SGs == 1
    OPT.load_amplitude = -250 # The magnitude of the applied unit load F (N)
    else
    OPT.load_amplitude = -5000 # The magnitude of the applied unit load F (N)
    end
  end
  # =======================================================================        
