function setup_bcs_cracked_plate2d(OPT::OPT_struct, FE::FE_struct)
    ## Input file 
    #
    ...
    load_nodes = findall(Bool.(FE.coords[1, :] .> (W - tol)) .&& Bool.(FE.coords[2, :] .> H - d - tol))
    sym_nodes = findall(Bool.(FE.coords[1, :] .< tol) .&& Bool.(FE.coords[2, :] .< H / 2 + tol))
    ...
    disp_dirs_s1 = vec(ones(1, length(disp_region1)))
    ...
    disp_region = vec([disp_region2; disp_region1])'
    disp_dirs = vec([disp_dirs_bl2; disp_dirs_s1])'
    disp_mag = vec([disp_mag2; disp_mag1])'
