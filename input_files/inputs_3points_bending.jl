function inputs_3points_bending(OPT::OPT_struct, FE::FE_struct)
    ## ============Input file================
    FE.mesh_input.type = "generate"
    if OPT.fd_check == 1
        FE.mesh_input.box_dimensions = [70, 30, 30] # ***
        FE.mesh_input.elements_per_side = Int.(FE.mesh_input.box_dimensions / 5)
    else
        FE.mesh_input.box_dimensions = [70, 30, 30]
        FE.mesh_input.elements_per_side = Int.(FE.mesh_input.box_dimensions / 5)
    end
    ...
    # Specify the Young"s modulus and Poisson ratio 
    FE.material.E = 115e3   # Young"s modulus of the design material
    FE.material.nu = 0.31 # Poisson ratio of the design material
    FE.material.rho_min = 1e-3 # Minimum density in void region
    FE.material.nu_void = 0.31 # Poisson ratio of the void material
    # ========
    OPT.options.obj_tol = 1e-5
    OPT.options.max_GRF = 0.15
    # ====
    OPT.parameters.init_dens_macro = 0.98  # initial macro density
    OPT.parameters.init_thickness_micro = 0.1  # initial micro THICKNESS
    ...
    OPT.aggregation_power = 12    # the power for the p-norm
    # ==========================================================================
    if OPT.fd_check == 1
        OPT.load_amplitude = -100 # The magnitude of the applied unit load F (N)
    end
    if OPT.run_SGs == 1 & OPT.run_stress == 1
        OPT.load_amplitude = -1000e3 # The magnitude of the applied unit load F (N) 
    end
    if OPT.run_SGs == 0 || OPT.run_stress == 0
  ...
  volume_fraction_limit = 0.4                # Volume fraction limit's value
  ...
  if OPT.run_multi_scales == 1
    ...
    if OPT.run_stress == 1
      ...
      OPT.functions.stress_limit = [828]   # MPa Stres Limit for stress constraints !!!
