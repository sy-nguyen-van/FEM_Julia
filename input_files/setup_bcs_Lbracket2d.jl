function setup_bcs_Lbracket2d(OPT::OPT_struct, FE::FE_struct)
    ## Input file 
    #
    ...
    LR_pt = findall(Bool.(FE.coords[1, :] .> (l - tol)) .&& Bool.(FE.coords[2, :] .< (1 / 8) * (l - c) + tol) .&& Bool.(FE.coords[2, :] .> -tol))
    ...
    ## Applied forces
    # Load case 1 
    net_mag1 = -1*OPT.load_amplitude  # Force magnitude (net over all nodes where applied)
