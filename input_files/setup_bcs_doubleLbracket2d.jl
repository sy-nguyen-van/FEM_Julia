function setup_bcs_doubleLbracket2d(OPT::OPT_struct, FE::FE_struct)
    ## Input file 
    #
    ...
    TR_pt  = findall( Bool.( FE.coords[1,:] .> (W-tol)) .&& Bool.(FE.coords[2,:] .> (7/8)*(H-c) - tol) .&& (FE.coords[2,:] .< H-c + tol) )
    TL_pt = findall( Bool.( FE.coords[1,:] .< tol) .&& Bool.(FE.coords[2,:] .> (7/8)*(H-c) - tol) .&& (FE.coords[2,:] .< H-c + tol) )
    T_edge = findall(Bool.(FE.coords[2,:] .> H - tol))
    ...
    # Load case 2 
    net_mag2 = net_mag1  # Force magnitude (net over all nodes where applied)
    load_dir2 = 2   # Force direction 

    load_region2 = TL_pt # LR_pt LL_pt
