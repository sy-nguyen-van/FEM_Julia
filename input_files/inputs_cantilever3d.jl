function inputs_cantilever3d(OPT::OPT_struct, FE::FE_struct)
  ## ============Input file================
  FE.mesh_input.type = "Abaqus_Mesh"

  if OPT.fd_check == 1
    FE.mesh_input.box_dimensions = [10, 4, 2]; # ***
    FE.mesh_input.elements_per_side = [10, 4, 2]; # ***
  else
    FE.mesh_input.box_dimensions = [120, 24, 12];  
    FE.mesh_input.elements_per_side = FE.mesh_input.box_dimensions;  
  end
  ## =======================================================================
  ...
  OPT.aggregation_power = 14    # the power for the p-norm
  # ==========================================================================
  if OPT.fd_check == 1
    OPT.load_amplitude = -10 # The magnitude of the applied unit load F (N)
  else
    if OPT.run_SGs == 1
    OPT.load_amplitude = -900 # The magnitude of the applied unit load F (N)
    else
    OPT.load_amplitude = -2000 # The magnitude of the applied unit load F (N)
    end
  end
  # =======================================================================       
