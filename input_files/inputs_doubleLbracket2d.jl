function inputs_doubleLbracket2d(OPT::OPT_struct, FE::FE_struct)
  ## ============Input file================
  FE.mesh_input.type = "doubleLbracket2d"
  
  if OPT.fd_check == 1
  FE.mesh_input.L_width = 8;
  FE.mesh_input.L_height = 5;
  FE.mesh_input.L_cutout = 3;
  FE.mesh_input.L_element_size = 1;
  else
  FE.mesh_input.L_width = 160;
  FE.mesh_input.L_height = 100;
  ...
  OPT.aggregation_power = 16    # the power for the p-norm
   # ========================================================================
   if OPT.fd_check == 1
    OPT.load_amplitude = -10 # The magnitude of the applied unit load F (N)
  else
    if OPT.run_SGs == 1
    OPT.load_amplitude = -150 # The magnitude of the applied unit load F (N)
    else
    OPT.load_amplitude = -1500 # The magnitude of the applied unit load F (N)
    end
  end
  # =======================================================================        
  ...
  OPT.functions.constraints_scale = 5.0      # Scale factor for constraints function
  ...
  if OPT.run_multi_scales == 1
    ...
    if OPT.run_stress == 1
      ...
      OPT.functions.stress_limit = [1080,1000]   # MPa Stres Limit for stress constraints !!!
  ...
  OPT.parameters.penalization_param = 4
