function inputs_cracked_plate2d(OPT::OPT_struct, FE::FE_struct)
  ## ============Input file================
  ...
  if OPT.fd_check == 1
    FE.mesh_input.box_dimensions = [5, 10] # 
    FE.mesh_input.elements_per_side = [5, 10] # 
  ...
  if OPT.fd_check == 1
    ...
    if OPT.run_SGs == 1
    OPT.load_amplitude = 800 # The magnitude of the applied unit load F (N)
    else
    OPT.load_amplitude = 6000 # The magnitude of the applied unit load F (N)
    end
  end
  # =======================================================================        
