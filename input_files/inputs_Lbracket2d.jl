function inputs_Lbracket2d(OPT::OPT_struct, FE::FE_struct)
  ...
  if OPT.fd_check == 1
    ...
    FE.mesh_input.L_cutout = 6
    FE.mesh_input.L_element_size = 1
  elseif OPT.fd_check == 0
    FE.mesh_input.L_side = 150
    FE.mesh_input.L_cutout = 90
    FE.mesh_input.L_element_size = 3  # Unit cell size = 1 mm
  end
  ...
  if OPT.fd_check == 1
    ...
    if OPT.run_SGs == 1
    OPT.load_amplitude = -30e3 # The magnitude of the applied unit load F (N)
    else
  ...
  OPT.parameters.filter_radius_factor = 3
  return OPT, FE
end
