function setup_bcs_3points_bending(OPT::OPT_struct, FE::FE_struct)
    ## Input file 
    #
    ...
    L = FE.mesh_input.box_dimensions[3]
    
    lw = 10
    
    tol = FE.max_elem_side / 1000

    load_nodes = findall( Bool.(FE.coords[1, :] .<   lw + tol) .&& Bool.(FE.coords[2, :] .> H-tol) )

    L_edge = findall(Bool.(FE.coords[1, :] .<   tol)  )

    roller_nodes = findall(Bool.(FE.coords[2, :] .< tol) .&& Bool.(FE.coords[1, :] .< (W-10) + lw/2 + tol)  .&&
     Bool.(FE.coords[1, :] .> (W-10) - lw/2- tol)  )

    R_corner = findall(   Bool.(FE.coords[1, :] .<  W + tol  ) .&&  Bool.(FE.coords[1, :] .>  W - tol  ).&& Bool.(FE.coords[3, :] .< tol  ) .&& Bool.(FE.coords[2, :] .<  tol)    ) 
    ...
    disp_region1 = L_edge
    disp_dirs1 = 1*ones(length(disp_region1)) # Along x-axis
    disp_mag1 = zeros(length(disp_region1))
    # Vertical roller on bottom-right point
    disp_region2 = roller_nodes
    disp_dirs2 = 2 * ones(length(disp_region2)) # Along y-axis
    disp_mag2 = zeros(length(disp_region2))
    # --
    disp_region3 = R_corner
    disp_dirs3 = 3*ones(length(disp_region3)) # Along x-axis
    disp_mag3 = zeros(length(disp_region3))
    # Combine displacement BC regions
    disp_region = vec([disp_region1; disp_region2; disp_region3])
    disp_dirs = vec([vec(disp_dirs1); vec(disp_dirs2); vec(disp_dirs3)])
    disp_mag = vec([vec(disp_mag1); vec(disp_mag2); vec(disp_mag3)])
