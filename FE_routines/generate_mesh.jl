...
function generate_mesh(FE::FE_struct)
    #
    ...
    x_i = [range(0, box_dimensions[i], n_i[i]) for i in 1:FE.dim]

    FE.n_elem = prod(elements_per_side[1:FE.dim])
    FE.n_node = prod(elements_per_side[1:FE.dim] .+ 1)

    if FE.dim == 2
        xx, yy = ndgrid(x_i[1], x_i[2])
        xx, yy= permutedims(xx, (2, 1)),permutedims(yy, (2, 1))
        node_coords = hcat(xx[:], yy[:])
    elseif FE.dim == 3
        xx, yy, zz = ndgrid(x_i[1], x_i[2], x_i[3])
        xx, yy, zz= permutedims(xx, (2, 1, 3)),permutedims(yy, (2, 1, 3)),permutedims(zz, (2, 1, 3))
        node_coords = hcat(xx[:], yy[:], zz[:])
    end
    ...
    if FE.dim == 2
        ...
        n3 = n2 .+ 1
        n4 = n1 .+ 1
        elem_mat = hcat(n1[:], n2[:], n3[:], n4[:])
    elseif FE.dim == 3
        nelz = elements_per_side[3]
        row = 1:1:nely
        col = (1:1:nelx)'
        pile = reshape(1:nelz, 1, 1, nelz)
        ...
        n7 = n6 .+ 1
        n8 = n5 .+ 1
        elem_mat = hcat(n1[:], n2[:], n3[:], n4[:], n5[:], n6[:], n7[:], n8[:])
    end

    ## export the mesh to the FE object by updating relevant fields 

    FE.coords = transpose(node_coords[:, 1:FE.dim])
    FE.elem_node = transpose(elem_mat)  # 4 nodes for quads, 8 for hexas

    ## print to terminal details of mesh generation

    @printf("generated %d cuboid mesh with %d elements and %d nodes.\n",
