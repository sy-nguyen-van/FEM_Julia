function genLbracketmesh(FE::FE_struct)
    ...
    if mod(W, h) > 0 || mod(S, h) > 0
        @show "Element size is not an exact divisor of L-bracket dimensions."
        return
    end
    ...
    # xcoords = 0:h:W
    # ycoords = 0:h:W
    xcoords = collect(0:h:W)
    ycoords = collect(0:h:W)
    inode = 0
    nnode_W = round(W / h) + 1
    nnode_S = round(S / h)
    nnodes = Int(nnode_W^2 - nnode_S^2)
    node = zeros(nnodes, 2)
    x_index = size(xcoords, 1)
    y_index = size(ycoords, 1)
    ...
    nelem_W = nnode_W - 1
    nelem_S = Int(nnode_S)
    nelem = Int(nelem_W^2 - nelem_S^2)
    elem = zeros(nelem, 4)

    # Vertical part of L-bracket + first column of elements of horizontal part
    for col in 1:nelem_W-nelem_S+1
        for row in 1:nelem_W
    ...
    for col in 1:nelem_S-1
        for row in 1:nelem_W-nelem_S
            ielem = ielem + 1
