function gendoubleLbracketmesh(FE::FE_struct)
    ...
    if mod(W, h) .> 0 || Bool(mod(S, h)) || mod(H, h) .> 0
        print("Element size is not an exact divisor of L-bracket dimensions.")
        return
    end
    # Numbering of nodes and elements goes bottom to top and then left to right
    # Nodes
    xcoords = 0:h:W
    ycoords = reverse(0:h:H)
    # X, Y = Matlab.meshgrid(xcoords, ycoords)

    X, Y = ndgrid(xcoords, ycoords)
    X, Y= permutedims(X, (2, 1)),permutedims(Y, (2, 1))

    inode = 0
    nnode_x = Int(round(W / h) + 1)
    nnode_y = Int(round(H / h) + 1)
    nnode_S = round(S / h)
    nnodes = Int((nnode_x * nnode_y) - 2 * nnode_S^2)
    node = zeros(nnodes, 2)
    # Array with node numbers
    N = zeros(size(X))
    for col in 1:nnode_x
        for row in 1:nnode_y
            thisx = X[row, col]
            thisy = Y[row, col]
    ...
    nelem_y = nnode_y - 1
    nelem_S = nnode_S
    nelem = Int(nelem_x * nelem_y - 2 * nelem_S^2)
    elem = zeros(nelem, 4)

    for col in 1:nelem_x
        for row in 1:nelem_y
    ...
    coords = CSV.read("coords.csv", DataFrame, header=false)
    elem_node = CSV.read("elem_node.csv", DataFrame, header=false)

    FE.coords = Matrix(coords)
    FE.elem_node = Matrix(elem_node)
